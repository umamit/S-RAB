...
import TakeOffModal from "@/components/TakeOffCalculator/TakeOffModal";
...
export default function TableItemRow({
  ...
  const { deleteItem, updateItem } = useRABStore();
    ...
    <>
      <tr className="hidden sm:table-row hover:bg-zinc-50/40 dark:hover:bg-zinc-900/10 group/row text-zinc-700 dark:text-zinc-300 font-medium">
        ...
        <td className="py-2.5 px-4 text-right">
          {isEditing("quantity") ? mkInp("quantity", "number") : (
            <div className="flex items-center justify-end gap-1 group/qty">
              <button type="button" onClick={() => setIsTakeOffOpen(true)} title="Kalkulator Dimensi Cepat"
                className="text-zinc-400 hover:text-blue-600 dark:hover:text-blue-400 opacity-0 group-hover/qty:opacity-100 p-0.5 transition-all">
                <Calculator className="w-3 h-3" />
              </button>
              <div onClick={() => handleCellClick(item, "quantity")} className="cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-900/60 rounded-sm py-0.5">{item.quantity}</div>
            </div>
          )}
        </td>
      ...
      <MobileItemCard
        project={project} activeSubProject={activeSubProject} category={category} item={item}
        index={index} itemWeight={itemWeight} isEditing={isEditing} mkInp={mkInp}
        handleCellClick={handleCellClick} setAhspItem={setAhspItem} deleteItem={deleteItem}
        onOpenTakeOff={() => setIsTakeOffOpen(true)}
      />

      <TakeOffModal
        isOpen={isTakeOffOpen}
        onClose={() => setIsTakeOffOpen(false)}
        itemName={item.name}
        itemUnit={item.unit}
        onApplyVolume={(vol) => {
          updateItem(project.id, activeSubProject.id, category.id, item.id, {
            quantity: vol,
          });
        }}
      />
