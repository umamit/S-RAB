...
export default function ProgressKPI({
  ...
  const isCpiHealthy = cpi >= 1;
  const isSpiHealthy = spi >= 1;
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Earned Value Analysis KPIs */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* CPI */}
        <div className={`p-4 rounded-xl border flex flex-col justify-between ${
          isCpiHealthy
            ? "border-green-200 bg-green-50/10 dark:border-green-950/10 dark:bg-green-950/5"
            : "border-red-200 bg-red-50/10 dark:border-red-950/10 dark:bg-red-950/5"
        }`}>
          <span className="text-[10px] uppercase font-bold text-zinc-400 tracking-wider">Indeks Efisiensi Biaya (CPI)</span>
          <div className="flex justify-between items-baseline mt-1">
            <span className={`text-xl font-extrabold ${isCpiHealthy ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
              {cpi.toFixed(2)}
            </span>
            <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${
              isCpiHealthy
              ...
              {isCpiHealthy ? "Hemat / Under-budget" : "Pemborosan / Over-budget"}
            </span>
          </div>
        </div>
        ...
        <div className={`p-4 rounded-xl border flex flex-col justify-between ${
          ...
          <span className="text-[10px] uppercase font-bold text-zinc-400 tracking-wider">Indeks Efisiensi Waktu (SPI)</span>
          <div className="flex justify-between items-baseline mt-1">
            <span className={`text-xl font-extrabold ${isSpiHealthy ? "text-green-600 dark:text-green-400" : "text-red-600 dark:text-red-400"}`}>
              {spi.toFixed(2)}
            </span>
            <span className={`text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded ${
              isSpiHealthy
              ...
              {isSpiHealthy ? "Cepat / Ahead of Schedule" : "Lambat / Behind Schedule"}
            </span>
          </div>
        </div>
      </div>
