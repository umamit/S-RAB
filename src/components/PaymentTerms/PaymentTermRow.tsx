...
export default function PaymentTermRow({ term, projectId, currentActualProgress }: PaymentTermRowProps) {
  ...
  const [paidDate, setPaidDate] = useState(term.paidDate || new Date().toISOString().split("T")[0]);
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[10px] text-zinc-500">
            Target progres: <strong>{term.targetProgress}%</strong>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        {editingDate && !term.isPaid && (
          <div className="flex items-center gap-2">
            ...
            />
            <button onClick={handleConfirmPaid} className="px-3 py-1 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg text-xs">
              Konfirmasi
            </button>
            <button onClick={() => setEditingDate(false)} className="text-xs text-zinc-400 hover:text-zinc-700">
              Batal
            </button>
          </div>
        )}
          ...
          <div className="flex items-center gap-2">
            {term.isPaid && (
              <span className="text-[10px] text-emerald-600 font-semibold flex items-center gap-1">
                <CheckCircle2 className="w-3 h-3" /> Lunas {term.paidDate}
              </span>
            )}
            {!term.isPaid && isReached && (
              <span className="text-[10px] text-amber-600 font-semibold flex items-center gap-1">
                <Zap className="w-3 h-3" /> Progres tercapai
              </span>
            )}
            <button
            ...
            <button
              ...
              className="text-zinc-300 hover:text-red-500 dark:text-zinc-700 dark:hover:text-red-400 transition-colors p-1"
            >
